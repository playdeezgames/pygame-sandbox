...
AREAS = [pygame.Rect((i % SPRITE_COLUMNS) * CELL_WIDTH, (i // SPRITE_COLUMNS) * CELL_HEIGHT, CELL_WIDTH, CELL_HEIGHT) for i in range(SPRITE_COUNT)]
DESTINATIONS = [((i % SPRITE_COLUMNS) * CELL_WIDTH + BORDER_X, (i // SPRITE_COLUMNS) * CELL_HEIGHT + BORDER_Y) for i in range(CELL_COUNT)]
CHARACTER_TABLE = {
    "@": 64,
    "A": 65,
    "B": 66,
...
def write(text):
    global buffer_index
    for char in text:
        put(buffer_index, CHARACTER_TABLE[char])
        buffer_index += 1
        buffer_index %= CELL_COUNT
